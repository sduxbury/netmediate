...
MEMS_pooled_ergm_param <- function(model,
                            micro_process,
                            macro_function,
  ...
  for(i in 1:length(model)){
    coef_list[[i]]<-model[[i]]$coefficients
    cov_list[[i]]<-model[[i]]$covar
    if(any(is.na(coef_list[[i]]))|
       any(is.infinite(coef_list[[i]]))){
      stop("Infinite or missing values in parameter estimates. Algorithm cannot continue.")

    }

    if(any(is.na(cov_list[[i]]))|
       any(is.infinite(cov_list[[i]]))){
      stop("Infinite or missing values in covariance matrix estimates. Algorithm cannot continue.")

    }

    theta_list[[i]]<-MASS::mvrnorm(n=nsim,
                         mu=coef_list[[i]],
                         Sigma=cov_list[[i]],
                         empirical = TRUE)
